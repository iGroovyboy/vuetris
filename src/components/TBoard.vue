...
export default {
  ...
  setup(props) {
    ...
    const newBlockEvent = () => { status.value = 'play'; blockColor.value = randomColor() };

    const collisionEvent = () => { status.value = 'collision'; };
    const scoreEvent = () => { status.value = 'score' };
    const gamePauseEvent = () => { status.value = 'pause' };
    const gamePlayEvent = () => { status.value = 'play' };
    const gameStopEvent = () => { status.value = 'stop' };
    ...
    return {
      ...
      scoreEvent, gamePauseEvent, gameStopEvent, gamePlayEvent, collisionEvent
    }
...
.stop .cell.borderY, .stop .cell.borderX {
  animation: borderCycle 4s infinite linear;
}
.stop .cell.block, .stop .cell.empty {
  background-color: transparent!important;
  border: 1px solid black;
}

.play .cell.borderY, .play .cell.borderX {
  background-color: orange;
}
.score .cell.borderY, .score .cell.borderX {
